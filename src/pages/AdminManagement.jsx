...
function AdminManagement() {
  ...
  // ✅ 【売上救出ロジック】Salesテーブルを優先し、無い日は Reservations から補完して表示
  const analyticsData = useMemo(() => {
    ...
    // 予約IDをキーにして、Salesテーブルに既に存在するデータは除外（二重計上防止）
    const salesReservationIds = new Set(salesRecords.map(s => s.reservation_id));

    // A. 過去データ（Reservations）の集計
    allReservations.filter(r => r.res_type === 'normal' && r.status === 'completed' && !salesReservationIds.has(r.id)).forEach(r => {
      const d = new Date(r.start_time);
      if (d.getFullYear() === currentYear) {
        months[d.getMonth()].total += (r.total_price || 0);
        months[d.getMonth()].count += 1;
        if (d.getMonth() === currentMonth) {
          days[d.getDate() - 1].total += (r.total_price || 0);
    ...
    salesRecords.forEach(s => {
      ...
      if (d.getFullYear() === currentYear) {
        months[d.getMonth()].total += (s.total_amount || 0);
        months[d.getMonth()].count += 1;
        if (d.getMonth() === currentMonth) {
          days[d.getDate() - 1].total += (s.total_amount || 0);
          days[d.getDate() - 1].count += 1;
        }
      }
    });
  ...
  const saveAllMasters = async () => {
    ...
    try {
      ...
      alert("設定を保存しました。"); fetchInitialData();
    } catch (err) { alert("保存失敗: " + err.message); } finally { setIsSaving(false); }
  };

  const addAdjustment = (svcId = null) => {
    const name = prompt("項目名を入力"); if (!name) return;
    let cat = null; if (svcId === null) cat = prompt("カテゴリ名を入力", "その他") || "その他";
  ...
  const addProduct = () => {
    const name = prompt("商品名を入力"); if (name) setProducts([...products, { id: crypto.randomUUID(), name, price: 0 }]);
  };
  ...
  const openCheckout = (res) => {
    ...
    setSelectedRes(res); setCheckoutServices(info.items); setCheckoutAdjustments([]); setCheckoutProducts([]); setFinalPrice(info.totalPrice); setOpenAdjCategory(null); setIsCheckoutOpen(true); setIsCustomerInfoOpen(false);
  };
  ...
  const toggleCheckoutService = (svc) => {
    ...
    setCheckoutServices(newSelection); calculateFinalTotal(newSelection, checkoutAdjustments, checkoutProducts);
  };
  ...
  const toggleCheckoutAdj = (adj) => {
    ...
    setCheckoutAdjustments(newSelection); calculateFinalTotal(checkoutServices, newSelection, checkoutProducts);
  };
  ...
  const toggleCheckoutProduct = (prod) => {
    ...
    setCheckoutProducts(newSelection); calculateFinalTotal(checkoutServices, checkoutAdjustments, newSelection);
  };
  ...
  const completePayment = async () => {
    try {
      ...
      const { error: resError } = await supabase.from('reservations').update({ 
        total_price: finalPrice, status: 'completed', total_slots: totalSlots, end_time: endTime.toISOString(), menu_name: checkoutServices.map(s => s.name).join(', '), options: { services: checkoutServices }
      ...
      const { error: salesError } = await supabase.from('sales').insert([{
        shop_id: cleanShopId, reservation_id: selectedRes.id, customer_id: cust?.id || null, total_amount: finalPrice, sale_date: selectedDate, payment_method: 'cash', details: { services: checkoutServices, products: checkoutProducts, adjustments: checkoutAdjustments }
      }]);
  ...
  const saveCustomerInfo = async () => {
    ...
    try {
      ...
      if (duplicate) {
        const confirmMerge = window.confirm(`「${editName}」様を統合しますか？`);
        if (confirmMerge) {
  ...
  const dailyTotalSales = useMemo(() => {
    // 日次売上も Reservations ではなく Sales ベースにする（移行期間は両方見るが、ここでは簡易化）
    return allReservations.filter(r => r.start_time.startsWith(selectedDate) && r.res_type === 'normal' && r.status === 'completed').reduce((sum, r) => sum + (r.total_price || 0), 0);
        ...
        {activeMenu === 'analytics' && (
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f8fafc' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '25px', display: 'flex', flexDirection: 'column', gap: '25px' }}>
              ...
              <div style={cardStyle}>
                <div style={catHeaderStyle}><span style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>🗓️ 年間・月別集計 ({viewMonth.getFullYear()}年)</span></div>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f8fafc' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '30px' }}>
              ...
              <div style={{ ...cardStyle, border: '3px solid #008000' }}>
                ...
                <div style={{ padding: '20px' }}>{products.map(p => (<div key={p.id} style={{ ...svcRowStyle, borderBottom: '1px solid #eee' }}><input value={p.name} onChange={(e) => setProducts(products.map(x => x.id === p.id ? {...x, name: e.target.value} : x))} style={{ ...optInputStyle, width: '200px' }} /><input type="number" value={p.price || 0} onChange={(e) => setProducts(products.map(x => x.id === p.id ? {...x, price: parseInt(e.target.value)} : x))} style={priceInputStyle} /><button onClick={() => { setDeletedProductIds([...deletedProductIds, p.id]); setProducts(products.filter(x => x.id !== p.id)); }} style={{ color: '#ef4444', border: 'none', background: 'none' }}><Trash2 size={18} /></button></div>))}</div>
              </div>
          ...
          <div style={{ ...checkoutPanelStyle, background: '#fdfcf5' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              ...
              <div style={{ background: '#fff', padding: '15px', borderRadius: '10px', border: '1px solid #eee', marginBottom: '20px' }}>
                <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#666' }}>👤 お客様名</label><input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} style={editInputStyle} />
                <div style={{ display: 'flex', gap: '10px' }}><div style={{ flex: 1 }}><label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#666' }}>📞 電話番号</label><input type="tel" value={editPhone} onChange={(e) => setEditPhone(e.target.value)} style={editInputStyle} /></div><div style={{ flex: 1 }}><label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#666' }}>🗓️ 初回来店日</label><input type="date" value={firstArrivalDate} onChange={(e) => setFirstArrivalDate(e.target.value)} style={editInputStyle} /></div></div>
                <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#666' }}>📧 メールアドレス</label><input type="email" value={editEmail} onChange={(e) => setEditEmail(e.target.value)} style={editInputStyle} />
              ...
              <button onClick={saveCustomerInfo} disabled={isSavingMemo} style={{ width: '100%', padding: '15px', background: '#008000', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 'bold', fontSize: '1rem', cursor: 'pointer', marginBottom: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>{isSavingMemo ? <RefreshCw className="animate-spin" size={18} /> : <Save size={18} />} 名簿・カルテ保存</button>
              <SectionTitle icon={<History size={16} />} title="過去の履歴" color="#4b2c85" />
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{pastVisits.length > 0 ? pastVisits.map(v => (<div key={v.id} style={{ background: '#fff', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }}><div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}><b>{v.start_time.split('T')[0]}</b><span style={{color:'#d34817'}}>¥{Number(v.total_price || 0).toLocaleString()}</span></div><p style={{ margin: 0, fontSize: '0.8rem', color: '#666' }}>{parseReservationDetails(v).menuName}</p></div>)) : <p style={{ textAlign: 'center', fontSize: '0.8rem', color: '#999', padding: '20px' }}>履歴なし</p>}</div>
            </div>
            <div style={{ padding: '25px', borderTop: '2px solid #ddd' }}><button onClick={() => openCheckout(selectedRes)} style={{ ...completeBtnStyle, background: '#d34817' }}><Clipboard size={20} /> お会計へ</button></div>
          </div>
        </div>
      )}
      ...
      {isCheckoutOpen && (
        <div style={checkoutOverlayStyle} onClick={() => setIsCheckoutOpen(false)}>
          <div style={checkoutPanelStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #4b2c85', paddingBottom: '5px', marginBottom: '15px' }}><div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#4b2c85', fontWeight: 'bold' }}><Clipboard size={16} /> 施術内容</div><button onClick={() => setIsMenuPopupOpen(true)} style={{ background: '#f3f0ff', color: '#4b2c85', border: '1px solid #4b2c85', borderRadius: '5px', padding: '2px 10px', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer' }}><Edit3 size={12} /> 変更</button></div>
              <div style={{ background: '#f9f9ff', padding: '15px', borderRadius: '10px', marginBottom: '25px', border: '1px dashed #4b2c85' }}>
                ...
                <div style={{ fontSize: '0.8rem', color: '#666', marginTop: '5px', display: 'flex', justifyContent: 'space-between' }}><span>時間: {checkoutServices.reduce((sum, s) => sum + (Number(s.slots) || 1), 0) * (shop?.slot_interval_min || 15)} 分</span><span style={{ color: '#4b2c85', fontWeight: 'bold' }}>¥ {checkoutServices.reduce((sum, s) => sum + (Number(s.price) || 0), 0).toLocaleString()}</span></div>
              </div>
              ...
              {(() => {
                ...
                const serviceSpecificAdjs = adminAdjustments.filter(adj => adj.service_id !== null && reservationServiceIds.includes(adj.service_id));
                return serviceSpecificAdjs.length > 0 && (
                  <div style={{ marginBottom: '15px', padding: '10px', background: '#fff5f5', borderRadius: '8px', border: '1px solid #feb2b2' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                      {serviceSpecificAdjs.map(adj => {
                        const isActive = checkoutAdjustments.some(a => a.id === adj.id);
                        return <button key={adj.id} onClick={() => toggleCheckoutAdj(adj)} style={adjBtnStyle(isActive)}>{adj.name} ({adj.is_minus ? '-' : ''}¥{adj.price})</button>;
                      })}
                    </div>
                  </div>
                );
              ...
              <div style={{ marginTop: '30px' }}><SectionTitle icon={<ShoppingBag size={16} />} title="店販商品を追加" color="#008000" /><div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>{products.map(prod => (<button key={prod.id} onClick={() => toggleCheckoutProduct(prod)} style={{ ...adjBtnStyle(checkoutProducts.some(p => p.id === prod.id)), borderColor: '#008000', color: checkoutProducts.some(p => p.id === prod.id) ? '#fff' : '#008000', background: checkoutProducts.some(p => p.id === prod.id) ? '#008000' : '#fff' }}>{prod.name} (¥{prod.price.toLocaleString()})</button>))}</div></div>
            </div>
            <div style={checkoutFooterStyle}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}><span style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>最終合計金額</span><span style={{ fontSize: '2.2rem', fontWeight: '900', color: '#d34817' }}>¥ {finalPrice.toLocaleString()}</span></div><button onClick={completePayment} style={completeBtnStyle}><CheckCircle size={20} /> 確定して台帳に記録</button></div>
          </div>
        </div>
      )}
