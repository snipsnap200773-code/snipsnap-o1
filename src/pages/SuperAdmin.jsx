...
function SuperAdmin() {
  ...
  const [editImageUrl, setEditImageUrl] = useState(''); // 店舗画像URLを追加
  ...
  // 💡 スマホ専用：はみ出しを物理的に不可能にするスタイル設定
  const containerStyle = { padding: '15px', fontFamily: 'sans-serif', maxWidth: '600px', margin: '0 auto', backgroundColor: '#f4f7f9', minHeight: '100vh', paddingBottom: '100px', boxSizing: 'border-box' };
  ...
  const flexRowStyle = { display: 'flex', gap: '10px', flexDirection: isMobile ? 'column' : 'row', marginBottom: '10px' };
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
          <div style={flexRowStyle}>
            ...
            <input value={newOwnerNameKana} onChange={(e) => setNewOwnerNameKana(e.target.value)} placeholder="氏名 かな" style={inputStyle} />
          </div>
          <div style={flexRowStyle}>
            <input value={newShopName} onChange={(e) => setNewShopName(e.target.value)} placeholder="店舗名" style={inputStyle} />
            <input value={newShopKana} onChange={(e) => setNewShopKana(e.target.value)} placeholder="店舗 かな" style={inputStyle} />
          </div>
          <select value={newBusinessType} onChange={(e) => setNewBusinessType(e.target.value)} style={{ ...inputStyle, marginBottom: '10px' }}>
            <option value="">-- 業種を選択 --</option>
            <option value="美容室・理容室">美容室・理容室</option>
            <option value="その他">その他</option>
          </select>
          <input value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder="メールアドレス" style={{ ...inputStyle, marginBottom: '10px' }} />
          <input value={newPhone} onChange={(e) => setNewPhone(e.target.value)} placeholder="電話番号" style={{ ...inputStyle, marginBottom: '10px' }} />
          <div style={{ padding: '12px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0', marginBottom: '10px' }}>
          ...
          <button onClick={createNewShop} style={{ padding: '16px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer', fontSize: '1rem' }}>店舗を発行する</button>
        </div>
      </div>
          ...
          <div key={shop.id} style={{ ...cardStyle, opacity: shop.is_suspended ? 0.7 : 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
              <div style={{ flex: 1 }}>
                ...
                {editingShopId === shop.id ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '10px' }}>
                    <div style={flexRowStyle}><input value={editOwnerName} onChange={(e) => setEditOwnerName(e.target.value)} style={inputStyle} /><input value={editOwnerNameKana} onChange={(e) => setEditOwnerNameKana(e.target.value)} style={inputStyle} /></div>
                    <div style={flexRowStyle}><input value={editName} onChange={(e) => setEditName(e.target.value)} style={inputStyle} /><input value={editKana} onChange={(e) => setEditKana(e.target.value)} style={inputStyle} /></div>
                    <input value={editEmail} onChange={(e) => setEditEmail(e.target.value)} style={inputStyle} placeholder="メール" />
                    ...
                    <div style={{ background: '#fef3c7', padding: '10px', borderRadius: '8px' }}>
                        <label style={{ fontSize: '0.65rem', fontWeight: 'bold' }}>管理用PW</label>
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                      ...
                      <button onClick={() => setEditingShopId(null)} style={{ flex: 1, padding: '12px', background: '#94a3b8', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold' }}>戻る</button>
                    </div>
                  </div>
                ) : (
                  <>
              ...
              {!editingShopId && (
                <div style={{ display: 'flex', gap: '5px' }}>
                  ...
                  <button onClick={() => deleteShop(shop)} style={{ ...iconBtnStyle, background: '#fef2f2', borderColor: '#fee2e2', color: '#ef4444' }}>消去</button>
                </div>
              )}
            </div>
            ...
            {!editingShopId && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <UrlBox label="💬 LINEメニュー用" url={`${window.location.origin}/shop/${shop.id}/reserve?openExternalBrowser=1`} color="#00b900" copy={() => copyToClipboard(`${window.location.origin}/shop/${shop.id}/reserve?openExternalBrowser=1`)} />
                <UrlBox label="📅 お客様予約用" url={`${window.location.origin}/shop/${shop.id}/reserve`} color="#059669" copy={() => copyToClipboard(`${window.location.origin}/shop/${shop.id}/reserve`)} />
                <button onClick={() => toggleSuspension(shop)} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: 'none', fontWeight: 'bold', background: shop.is_suspended ? '#10b981' : '#fee2e2', color: shop.is_suspended ? '#fff' : '#ef4444', fontSize: '0.85rem', cursor: 'pointer' }}>
...
// 💡 スマホでボタンが突き抜けないように設計したURLコンポーネント
const UrlBox = ({ label, url, color, copy }) => (
  <div style={{ boxSizing: 'border-box', width: '100%' }}>
    <label style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 'bold', display: 'block', marginBottom: '4px' }}>{label}</label>
    <div style={{ display: 'flex', gap: '5px' }}>
      <input readOnly value={url} style={{ flex: 1, padding: '10px', fontSize: '0.8rem', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', boxSizing: 'border-box' }} />
      <button onClick={copy} style={{ padding: '8px 12px', background: '#fff', border: `1px solid ${color}`, color, borderRadius: '8px', fontWeight: 'bold', fontSize: '0.75rem', cursor: 'pointer' }}>コピー</button>
...
const iconBtnStyle = { background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#475569', cursor: 'pointer', fontSize: '0.75rem', padding: '6px 12px', borderRadius: '8px', fontWeight: 'bold' };
