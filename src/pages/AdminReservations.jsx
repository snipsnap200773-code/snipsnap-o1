...
function AdminReservations() {
  ...
  const getStatusAt = (dateStr, timeStr) => {
    ...
    // 1. 実予約（normal/blocked）を最優先でチェックして詳細を保持
    ...
    if (shop?.auto_fill_logic && dayRes.length > 0) {
      const primeSeats = []; // 死守すべき特等席
      const gapCandidates = []; // ブロック候補

      dayRes.forEach(r => {
        // --- 後ろパズル：特等席の次（2マス目）をブロック ---
        const resEnd = new Date(r.end_time).getTime();
        ...
        if (sIdx >= 3) gapCandidates.push(timeSlots[sIdx - 3]);
      });
  ...
  // 🆕 修正：管理者による個別ブロック（start_at / end_at の完全補完）
  ...
  // 🆕 修正：臨時休業（start_at / end_at / total_slots の完全補完）
